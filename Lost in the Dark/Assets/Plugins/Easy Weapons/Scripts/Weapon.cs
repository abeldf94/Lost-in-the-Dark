...
public class SmartBulletHoleGroup
{
	...
	public SmartBulletHoleGroup(string t, Material m, PhysicMaterial pm)
	{
		tag = t;
		material = m;
		physicMaterial = pm;
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
                ...
                float damage = Random.Range(5f, 10f) * powerMultiplier;
                if (warmup)
				{
	...
	void Beam()
	{
		...
		do
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
                ...
                // Damage
				hit.collider.gameObject.SendMessageUpwards("Damage", -beamPower, SendMessageOptions.DontRequireReceiver);
